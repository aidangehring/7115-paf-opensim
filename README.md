...
The reason for this is that with a time constraint on the assignment, I did not have time to prepare the program to determine which foot is conencted to which force plate, and update the Opensim external loads file based on this detection. As such, I configured the external loads to only apply force through the right calcaneus at the rubber, and left calcaneus at landing.
